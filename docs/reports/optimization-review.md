...
- `python -m scripts.create_tables` (from `backend/`) creates missing tables without dropping data.
- `python -m scripts.reset_dev_db` (from `backend/`) destructively deletes all Qdrant collections and rebuilds all database tables for fresh testing.
...
cd backend
python -m compileall -q app tests scripts/create_tables.py scripts/reset_dev_db.py
