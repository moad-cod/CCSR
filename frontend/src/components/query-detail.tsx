...
export function QueryDetail({projectId, queryId}: {projectId: string; queryId: string}) {
  ...
  return <div className="mx-auto max-w-6xl space-y-6"><Link href={`/projects/${projectId}/playground?view=history`} className="inline-flex items-center gap-1.5 text-[10px] text-[var(--ink-muted)] hover:text-[var(--ink)]"><ArrowLeft className="size-3" />Playground history</Link><PageHeader eyebrow={project.data?.name ?? "Project"} title="Query detail" description="Inspect the persisted playground answer and every ranked retrieval record used to produce it." actions={item.answer ? <Button variant="secondary" size="sm" onClick={() => {void navigator.clipboard?.writeText(item.answer ?? ""); toast.success("Answer copied");}}><Copy className="size-3.5" />Copy answer</Button> : undefined} />
    <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><p className="text-[9px] font-semibold uppercase tracking-[.13em] text-[var(--ink-disabled)]">Question</p><h2 className="mt-2 text-base font-semibold leading-7">{item.question}</h2><div className="mt-4 flex flex-wrap gap-2">{item.provider ? <Badge tone="info">{item.provider}</Badge> : null}{item.model ? <Badge>{item.model}</Badge> : null}{item.cache_hit ? <Badge tone="success">cache hit</Badge> : <Badge>generated</Badge>}<Badge><Clock3 className="mr-1 size-3" />{formatLatency(item.latency_ms)}</Badge></div></section>
    <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><p className="text-[9px] font-semibold uppercase tracking-[.13em] text-[var(--ink-disabled)]">Persisted answer</p>{item.answer ? <div className="markdown-answer mt-4"><ReactMarkdown remarkPlugins={[remarkGfm]}>{item.answer}</ReactMarkdown></div> : <div className="mt-4 rounded-lg bg-[var(--danger-soft)] p-4 text-xs text-[var(--danger-soft-text)]">The provider failed before an answer was persisted.</div>}</section>
    <section><div className="mb-4 flex items-center justify-between"><div><h2 className="text-sm font-semibold">Retrieval trace</h2><p className="mt-1 text-[10px] text-[var(--ink-muted)]">{item.retrievals.length} ranked evidence records</p></div><FileSearch className="size-4 text-[var(--ink-disabled)]" /></div><RetrievalTrace items={item.retrievals} onSelect={(source) => {if (source.document_id) window.location.assign(`/projects/${projectId}/documents/${source.document_id}?chunk=${source.chunk_id ?? ""}`);}} /></section>
  </div>;
}
