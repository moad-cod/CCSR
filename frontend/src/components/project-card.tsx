...
export function ProjectCard({project, documentCount, activeRuns, view, onRename, onDelete}: {
  ...
  return <article className={cn("group relative rounded-xl border border-[var(--border)] bg-[var(--surface)] transition hover:border-[var(--accent-border)]", view === "grid" ? "p-5" : "flex items-center gap-4 p-4")}>
    ...
    <div className={cn("min-w-0 flex-1", view === "grid" && "mt-4")}>
      ...
      <p className={cn("text-[11px] leading-5 text-[var(--ink-secondary)]", view === "grid" ? "mt-3 line-clamp-2 min-h-10" : "mt-1 line-clamp-1")}>{hasRAGForge ? "Research records with an enabled RAGForge source, retrieval, and ingestion workspace." : "A capability-neutral workspace for research, experiments, artifacts, runs, and publications."}</p>
      <div className="mt-4 flex flex-wrap items-center gap-4 text-[10px] text-[var(--ink-muted)]">{hasRAGForge ? <><span className="flex items-center gap-1.5"><FileStack className="size-3" />{documentCount === null ? "…" : documentCount} sources</span><span className="flex items-center gap-1.5"><Activity className="size-3" />{activeRuns === null ? "…" : activeRuns ? `${activeRuns} active` : "No active runs"}</span></> : <span>Platform research</span>}</div>
    </div>
      ...
      {canWrite || canManage ? <DropdownMenu><Tooltip><TooltipTrigger asChild><DropdownMenuTrigger asChild><button className="icon-button" aria-label={`Actions for ${project.name}`}><MoreHorizontal className="size-4" /></button></DropdownMenuTrigger></TooltipTrigger><TooltipContent>Project actions</TooltipContent></Tooltip><DropdownMenuContent align="end">{canWrite ? <DropdownMenuItem onSelect={onRename}><Pencil className="size-3.5" />Rename</DropdownMenuItem> : null}{canManage ? <DropdownMenuItem className="text-[var(--danger)] data-[highlighted]:text-[var(--danger)]" onSelect={onDelete}><Trash2 className="size-3.5" />Delete</DropdownMenuItem> : null}</DropdownMenuContent></DropdownMenu> : null}
    </div>
  </article>;
}
