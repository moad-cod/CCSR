...
export function ProjectForm({organizations, chunkers, isPending, submitLabel = "Create project", initialName = "", onCancel, onSubmit}: {
  ...
  return <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
    <label className="block"><span className="mb-2 block text-xs font-medium">Project name</span><Input autoFocus placeholder="Retrieval evaluation study" {...register("name")} />{errors.name ? <span className="mt-1.5 block text-xs text-red-300">{errors.name.message}</span> : null}</label>
