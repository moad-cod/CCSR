  ...
  return <div className="relative flex h-full min-w-0 overflow-hidden">
    <main className="unified-workspace relative grid min-w-[320px] flex-1 grid-rows-[auto_minmax(0,1fr)] bg-[var(--background)]">
      ...
      <div className="absolute right-2 top-[57px] z-20 hidden gap-1 xl:flex">
        <button onClick={() => setLeftOpen((value) => !value)} title={leftOpen ? "Collapse sources" : "Open sources"} className="flex size-7 items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] text-[var(--ink-muted)] shadow-[var(--shadow-md)] hover:text-[var(--ink)]">{leftOpen ? <PanelRightClose className="size-3.5" /> : <PanelRightOpen className="size-3.5" />}</button>
      </div>
      <button onClick={() => setMobileDocuments(true)} className="absolute bottom-14 right-3 z-30 flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] px-2.5 text-xs text-[var(--ink-secondary)] shadow-[var(--shadow-md)] xl:hidden"><FileStack className="size-3" />Sources</button>
      <button disabled={!activeDocument} onClick={() => setWorkspaceTab("document")} className="absolute bottom-24 right-3 z-30 flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] px-2.5 text-xs text-[var(--ink-secondary)] shadow-[var(--shadow-md)] disabled:hidden xl:hidden"><PanelRightOpen className="size-3" />Source</button>
    </main>

    {leftOpen ? <div role="separator" aria-label="Resize source panel" aria-orientation="vertical" onPointerDown={startResize} className="relative z-30 hidden w-px shrink-0 cursor-col-resize bg-[var(--border)] transition hover:bg-[var(--accent-muted)] xl:block"><span className="absolute inset-y-0 -left-1.5 w-3" /></div> : null}
    <div className="knowledge-panel hidden h-full min-h-0 shrink-0 border-l border-[var(--border)] xl:block" style={{width: leftOpen ? `clamp(320px, 22vw, ${leftWidth}px)` : 0}}>
    ...
    {mobileDocuments ? <div className="fixed inset-0 z-[80] xl:hidden"><button aria-label="Close sources" className="absolute inset-0 bg-[var(--background-overlay)]" onClick={() => setMobileDocuments(false)} /><div className="absolute inset-y-0 right-0 w-[88%] max-w-[360px] border-l border-[var(--border)] shadow-[var(--shadow-lg)]"><DocumentPanel projectId={projectId} projectName={projectName} documents={documents} runs={runs} chunkers={chunkers} selectedIds={effectiveSelectedIds} selectedDocumentId={activeDocument?.document_id ?? ""} onSelectDocument={(id) => {setSelectedDocumentId(id); setMobileDocuments(false);}} onOpenDocument={(id) => {openDocument(id); setMobileDocuments(false);}} onToggleSelected={toggleSelected} onUpload={upload} onUrl={addUrl} onGDrive={addGDrive} onDelete={setDeleting} onRetry={(run) => retryRun.mutate(run)} /><button onClick={() => setMobileDocuments(false)} className="absolute left-2 top-2 flex size-7 items-center justify-center rounded-lg bg-[var(--surface-hover)]"><X className="size-3.5" /></button></div></div> : null}
