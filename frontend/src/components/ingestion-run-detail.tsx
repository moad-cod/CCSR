...
function LiveRun({initial, document, projectId}: {initial: IngestionRun; document?: Document; projectId: string}) {const {run, connected} = useIngestionStream(initial); return <><div className="mb-4 flex items-center gap-2 text-[10px] text-[var(--ink-muted)]"><span className={`size-2 rounded-full ${connected ? "bg-[var(--accent)]" : "bg-[var(--warning)]"}`} />{connected ? "Live pipeline updates connected" : "Recovering from durable status"}</div><IngestionPipeline run={run} document={document} projectId={projectId} /><div className="mt-5 grid gap-3 sm:grid-cols-3">{[[HardDrive,"Bronze artifact",run.progress.bronze],[Database,"Silver / Gold",run.progress.silver && run.progress.gold],[Network,"Qdrant index",run.progress.qdrant]].map(([Icon,label,ready]) => {const Component = Icon as typeof HardDrive; return <div key={String(label)} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4"><Component className={`size-4 ${ready ? "text-[var(--accent)]" : "text-[var(--ink-disabled)]"}`} /><p className="mt-3 text-xs font-medium">{String(label)}</p><p className="mt-1 text-[9px] text-[var(--ink-muted)]">{ready ? "Durable boundary completed" : "Not completed"}</p></div>;})}</div></>;}
