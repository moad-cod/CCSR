...
export function ObservabilityDashboard({projectId}: {projectId?: string}) {
  const overview = useWorkspaceOverview({documents: true, runs: true, history: true, enabled: !projectId});
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><PageHeader eyebrow={project.data?.name ?? "Control plane"} title="Observability" description="Real metrics derived from PostgreSQL document, ingestion, and query records. No estimated or synthetic values are shown." />
    ...
    <div className="grid gap-5 xl:grid-cols-2"><section className="rounded-xl border border-[var(--border)] bg-[var(--surface)]"><div className="flex items-center justify-between border-b border-[var(--border)] p-4"><div><h2 className="text-sm font-semibold">Recent ingestion</h2><p className="mt-1 text-[9px] text-[var(--ink-muted)]">{failedRuns} failed runs in the loaded window</p></div><Workflow className="size-4 text-[var(--ink-disabled)]" /></div><div className="divide-y divide-[var(--border)]">{runItems.slice(0,6).map((run) => {const targetProject = projectId ?? (run as IngestionRun & {project?: Project}).project?.project_id; return <Link key={run.ingestion_run_id} href={`/projects/${targetProject}/runs/${run.ingestion_run_id}`} className="flex items-center gap-3 p-4 hover:bg-[var(--surface-elevated)]"><span className="min-w-0 flex-1"><span className="mono block truncate text-[9px] text-[var(--ink-secondary)]">{run.ingestion_run_id}</span><span className="mt-1 block text-[8px] text-[var(--ink-disabled)]">{relativeTime(run.created_at)}</span></span><StatusBadge status={run.status} /></Link>;})}{!runItems.length ? <p className="p-8 text-center text-[10px] text-[var(--ink-muted)]">No ingestion runs yet.</p> : null}</div></section>
      <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)]"><div className="flex items-center justify-between border-b border-[var(--border)] p-4"><div><h2 className="text-sm font-semibold">Recent queries</h2><p className="mt-1 text-[9px] text-[var(--ink-muted)]">Durable provider and latency records</p></div><History className="size-4 text-[var(--ink-disabled)]" /></div><div className="divide-y divide-[var(--border)]">{queries.slice(0,6).map((query) => {const targetProject = projectId ?? (query as QueryHistoryItem & {project?: Project}).project?.project_id; return <Link key={query.query_log_id} href={`/projects/${targetProject}/history/${query.query_log_id}`} className="flex items-center gap-3 p-4 hover:bg-[var(--surface-elevated)]"><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-medium">{query.question}</span><span className="mt-1 flex items-center gap-1 text-[8px] text-[var(--ink-disabled)]"><Clock3 className="size-2.5" />{formatLatency(query.latency_ms)} · {query.provider ?? "provider unavailable"}</span></span><StatusBadge status={query.answer ? "answered" : "failed"} /></Link>;})}{!queries.length ? <p className="p-8 text-center text-[10px] text-[var(--ink-muted)]">No query history yet.</p> : null}</div></section></div>
  </div>;
}
